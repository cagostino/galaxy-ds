...
class SFRMatch:
    ...
    def get_highsn_match_only(self, agn_inds, sf_inds, sf_plus_inds, 
                              agnplus_inds, sncut=2, load=False, fname='', with_av=True, balmdecmin =-99):
        '''
        ...
        if not load:
            ...
            for i, agn_ind in enumerate(agn_inds):
                ...
                diffz_agn = (self.bpt_eldiag.z.iloc[agn_ind]-self.bpt_eldiag.z.iloc[agn_inds[otheragns]])**2/np.std(redshift_m2)
                ...
                diffz_agnplus = (self.bpt_eldiag.z.iloc[agn_ind]-self.plus_eldiag.z.iloc[agnplus_inds])**2/np.std(redshift_m2)
                ...
                diffz_bpt = (self.bpt_eldiag.z.iloc[agn_ind]-self.bpt_eldiag.z.iloc[sf_inds])**2/np.std(redshift_m2)
                ...
                inds_high_sn_bpt = np.where((oiiiflux_sn_sub_bpt.iloc[diffs_bpt_sort]>sncut) & 
                                            (niiflux_sn_sub_bpt.iloc[diffs_bpt_sort]>sncut) &
                                            (hbetaflux_sn_sub_bpt.iloc[diffs_bpt_sort]>sncut) &
                                            (halpflux_sn_sub_bpt.iloc[diffs_bpt_sort] >sncut))[0]# &
                ...
                diffz_bptplus = (self.bpt_eldiag.z.iloc[agn_ind]-self.plus_eldiag.z.iloc[sf_plus_inds])**2/np.std(redshift_m2)
                ...
                inds_high_sn_bptplus = np.where((oiiiflux_sn_sub_plus.iloc[diffs_bptplus_sort]>sncut) & 
                                                (niiflux_sn_sub_plus.iloc[diffs_bptplus_sort]>sncut) &
                                                (hbetaflux_sn_sub_plus.iloc[diffs_bptplus_sort]>sncut) &
                                                (halpflux_sn_sub_plus.iloc[diffs_bptplus_sort] >sncut)&
                ...
                diffz_neither = (self.bpt_eldiag.z.iloc[agn_ind]-self.neither_eldiag.z)**2/np.std(redshift_m2)
                ...
                inds_high_sn_neither = np.where((oiiiflux_sn_sub_neither.iloc[diffs_neither_sort]>sncut) & 
                                                (niiflux_sn_sub_neither.iloc[diffs_neither_sort]>sncut) &
                                                (hbetaflux_sn_sub_neither.iloc[diffs_neither_sort]>sncut) &
                                                (halpflux_sn_sub_neither.iloc[diffs_neither_sort] >sncut) &
    ...
    def subtract_elflux(self, sncut=2, halphbeta_sncut=2):
        ...
        #######################
        '''
        sfrm_gsw2.allagn_df[['corrected_av','niiha_sub', 'oiiihb_sub', 'siiha_sub','oiha_sub','oi_sii_sub','U_sub','vdisp','mbh','sfr',
        ...
        '''
        matches_groups = [self.sfs, self.sfs_plus, self.neither_matches]
        agn_groups = [self.agns, self.agns_plus, self.neither_agn]
        ...
        self.high_sn5_hb = np.where((self.allagn_df['hbetaflux_sub_sn']>5))        
        self.high_hb5_allagn_df = self.allagn_df.iloc[self.high_sn5_hb].copy()


        self.high_sn10_hb = np.where((self.allagn_df['hbetaflux_sub_sn']>10))        
        self.high_hb10_allagn_df = self.allagn_df.iloc[self.high_sn10_hb].copy()


        self.high_sn20_hb = np.where((self.allagn_df['hbetaflux_sub_sn']>20))        
        self.high_hb20_allagn_df = self.allagn_df.iloc[self.high_sn20_hb].copy()

        self.high_sn5_hb_ha = np.where((self.allagn_df['hbetaflux_sub_sn']>5) &(self.allagn_df['halpflux_sub_sn']>5))        
        self.high_hb_ha5_allagn_df = self.allagn_df.iloc[self.high_sn5_hb_ha].copy()


        self.high_sn10_hb_ha = np.where((self.allagn_df['hbetaflux_sub_sn']>10) &(self.allagn_df['halpflux_sub_sn']>10))        
        self.high_hb_ha10_allagn_df = self.allagn_df.iloc[self.high_sn10_hb_ha].copy()


        self.high_sn20_hb_ha = np.where((self.allagn_df['hbetaflux_sub_sn']>20) &(self.allagn_df['halpflux_sub_sn']>20))        
        self.high_hb_ha20_allagn_df = self.allagn_df.iloc[self.high_sn20_hb_ha].copy()
        ...
        self.x_pred_av = np.vstack([self.allagn_df.av_gsw]).transpose()
        self.x_pred_av_sfr = np.vstack([self.allagn_df.av_gsw, self.allagn_df.sfr]).transpose()
        self.x_pred_av_mass = np.vstack([self.allagn_df.av_gsw, self.allagn_df.mass]).transpose()
        self.x_pred_av_mass_sfr = np.vstack([self.allagn_df.av_gsw, self.allagn_df.mass, self.allagn_df.sfr]).transpose()

        self.allagn_df['corrected_av'] = correct_av(self.reg_av, self.x_pred_av, 
                                                         np.array(self.allagn_df.av_sub_agn),
                                                         np.array(self.allagn_df.hbetaflux_sub_sn))

        self.allagn_df['corrected_av_sfr'] = correct_av(self.reg_av_sfr, self.x_pred_av_sfr, 
                                                         np.array(self.allagn_df.av_sub_agn),
                                                         np.array(self.allagn_df.hbetaflux_sub_sn))

        self.allagn_df['corrected_av_mass'] = correct_av(self.reg_av_mass, self.x_pred_av_mass, 
                                                         np.array(self.allagn_df.av_sub_agn),
                                                         np.array(self.allagn_df.hbetaflux_sub_sn))

        self.allagn_df['corrected_av_mass_sfr'] = correct_av(self.reg_av_mass_sfr, self.x_pred_av_mass_sfr, 
                                                         np.array(self.allagn_df.av_sub_agn),
                                                         np.array(self.allagn_df.hbetaflux_sub_sn))

        self.allagn_df['halpflux_corr_sub'] = dustcorrect(self.allagn_df['halpflux_sub'], self.allagn_df['corrected_av'],6563.0)
        self.allagn_df['hbetaflux_corr_sub'] = dustcorrect(self.allagn_df['hbetaflux_sub'], self.allagn_df['corrected_av'],4861.0)

        self.allagn_df['halpflux_corrbd_sub'] = dustcorrect(self.allagn_df['halpflux_sub'], self.allagn_df['av_sub_agn'],6563.0)
        self.allagn_df['hbetaflux_corrbd_sub'] = dustcorrect(self.allagn_df['hbetaflux_sub'], self.allagn_df['av_sub_agn'],4861.0)


        self.allagn_df['oiiiflux_corr_sub'] = dustcorrect(self.allagn_df['oiiiflux_sub'], self.allagn_df['corrected_av'], 5007.0)
        self.allagn_df['oiii4959flux_corr_sub'] = dustcorrect(self.allagn_df['oiii4959flux_sub'], self.allagn_df['corrected_av'], 4959.0)
        self.allagn_df['oiii4363flux_corr_sub'] = dustcorrect(self.allagn_df['oiii4363flux_sub'], self.allagn_df['corrected_av'], 4363.0)



        self.allagn_df['niiflux_corr_sub'] = dustcorrect(self.allagn_df['niiflux_sub'], self.allagn_df['corrected_av'], 6583.0)
        
        self.allagn_df['siiflux_corr_sub'] = dustcorrect(self.allagn_df['siiflux_sub'], self.allagn_df['corrected_av'], 6724.0)
        self.allagn_df['sii6717flux_corr_sub'] = dustcorrect(self.allagn_df['sii6717flux_sub'], self.allagn_df['corrected_av'], 6717.0)
        self.allagn_df['sii6731flux_corr_sub'] = dustcorrect(self.allagn_df['sii6731flux_sub'], self.allagn_df['corrected_av'], 6731.0)
        self.allagn_df['oiiflux_corr_sub'] = dustcorrect(self.allagn_df['oiiflux_sub'], self.allagn_df['corrected_av'], 3727.0)
        self.allagn_df['oiflux_corr_sub'] = dustcorrect(self.allagn_df['oiflux_sub'], self.allagn_df['corrected_av'], 6300.0)
        
        self.allagn_df['oiiilum_sub_dered'] = np.log10(getlumfromflux(self.allagn_df['oiiiflux_corr_sub'], self.allagn_df['z']))
        self.allagn_df['halplum_sub_dered'] = np.log10(getlumfromflux(self.allagn_df['halpflux_corr_sub'], self.allagn_df['z']))

        self.allagn_df['oiiilum_uncorr_sub'] = getlumfromflux(self.allagn_df['halpflux_sub'], self.allagn_df['z'])
        self.allagn_df['halplum_uncorr_sub'] = getlumfromflux(self.allagn_df['oiiiflux_sub'], self.allagn_df['z'])

        self.allagn_df['halpfibsfr_sub'] = halptofibsfr_corr(10**self.allagn_df['halplum_sub_dered'])
        self.allagn_df['halphbeta_sub_ratio']= np.copy(np.array(self.allagn_df['halpflux_sub']/self.allagn_df['hbetaflux_sub']))
        self.allagn_df['halphbeta_sub_ratio_err']= np.copy(self.allagn_df['halphbeta_sub_ratio']*np.sqrt((self.allagn_df['halp_err_sub']/(self.allagn_df['halpflux_sub']))**2 + 
                                                            (self.allagn_df['hbeta_err_sub']/(self.allagn_df['hbetaflux_sub']))**2) )
        self.allagn_df['halphbeta_sub_ratio_sn']= np.copy(np.array(self.allagn_df['halphbeta_sub_ratio']/self.allagn_df['halphbeta_sub_ratio_err']))
        

        self.allagn_df['nii_oii_sub'] =np.log10(self.allagn_df['niiflux_corr_sub']/self.allagn_df['oiiflux_corr_sub'])
        self.allagn_df['log_oh_sub'] = nii_oii_to_oh(self.allagn_df['niiflux_corr_sub'],self.allagn_df['oiiflux_corr_sub'])         
        self.allagn_df['sii_ratio_sub'] =self.allagn_df['sii6717flux_sub']/self.allagn_df['sii6731flux_sub']
        self.allagn_df['sii_oii_sub'] =np.log10(self.allagn_df['siiflux_sub']/self.allagn_df['oiiflux_sub'])
        '''
        self.allagn_df['O32_sub'] = np.log10( (self.allagn_df['oiii4959flux_corr_sub'] + self.allagn_df['oiiiflux_corr_sub'])/self.allagn_df['oiiflux_corr_sub'])
        self.allagn_df['q_sub'] = nii_logoh_o32_to_q(self.allagn_df['log_oh_sub'], self.allagn_df['O32_sub'])
        self.allagn_df['qc_sub'] = self.allagn_df['q_sub']-np.log10(3e10)
        '''
        self.allagn_df['oiii_oii_sub'] =np.log10(self.allagn_df['oiiiflux_corr_sub']/self.allagn_df['oiiflux_corr_sub'])
        self.allagn_df['U_sub'] = oiii_oii_to_U(self.allagn_df['oiii_oii_sub'])
        
        sii_diag_oiiihb_exp =  np.array(np.log10(y2_linersy2(self.allagn_df['siiha_sub'])))
        sy2_liner = (np.array(self.allagn_df['oiiihb_sub'])>sii_diag_oiiihb_exp)
        sy2_liner[np.where(sy2_liner == True)[0]] = 1
        sy2_liner[np.where(sy2_liner == False)[0]] = 0
        self.allagn_df['sy2_liner_bool'] = sy2_liner        
    def get_filt_dfs(self, filts, gen_filts, 
        ...
        for i in range(len(line_filts)):
            line_sn_filt = np.where(self.allagn_df[line_filts[i][0]+'_sn']> line_filts[i][1])        
            self.gen_filts[line_filts[i][0]+'_filt'] = line_sn_filt
            self.gen_filts[line_filts[i][0]+'_df'] = self.allagn_df.iloc[line_sn_filt].copy()
            self.match_filts[line_filts[i][0]+'_df'] = self.allmatch_df.iloc[line_sn_filt].copy()
            
            
        for filt in self.filts.keys():
            if len(self.filts[filt]['cut'])==2:
                cent_filt = np.where((self.allagn_df[filt]>self.filts[filt]['cut'][0])  &
                                     (self.allagn_df[filt]<self.filts[filt]['cut'][1]))[0]
                self.gen_filts['mid_'+filt+'_filt'] = cent_filt
                self.gen_filts['mid_'+filt+'_df'] = self.allagn_df.iloc[cent_filt].copy()
                self.match_filts['mid_'+filt+'_df'] = self.allmatch_df.iloc[cent_filt].copy()
                 
        for comb in line_filts_comb.keys():
            n_comb = len(line_filts_comb[comb][0])
            init_ = [True]*len(self.allagn_df)
            for i in range(n_comb):
               
                mask = (self.allagn_df[line_filts_comb[comb][0][i]+'_sn']>line_filts_comb[comb][1])
                pass_mask = (mask & init_)
                init_ = pass_mask
            
            comb_sn_filt  = np.where(pass_mask)
            self.gen_filts['filt_comb_'+comb] = comb_sn_filt
            self.gen_filts['df_comb_'+comb] = self.allagn_df.iloc[comb_sn_filt].copy()            
            self.match_filts['df_comb_'+comb] = self.allmatch_df.iloc[comb_sn_filt].copy()            
                
        for filt in self.filts.keys():
            if len(self.filts[filt]['cut']) ==1:                

                up = np.where(self.allagn_df[filt] > self.filts[filt]['cut'][0])[0]
                down = np.where(self.allagn_df[filt] <= self.filts[filt]['cut'][0])[0]
                
                self.filts[filt]['up_filt'] = up
                self.filts[filt]['up_df'] = self.allagn_df.iloc[up].copy()
                self.match_filts[filt]['up_df'] = self.allmatch_df.iloc[up].copy()
                
                self.filts[filt]['down_filt'] = down
                self.filts[filt]['down_df'] = self.allagn_df.iloc[down].copy()
                self.match_filts[filt]['down_df'] = self.allmatch_df.iloc[down].copy()

                uphamatch = np.where((np.array(self.allagn_df[filt]) > self.filts[filt]['cut'][0])&
                                     (np.array(self.allmatch_df['halpflux_sn'])>sncut))[0]
                downhamatch = np.where((np.array(self.allagn_df[filt]) <= self.filts[filt]['cut'][0])&
                                       (np.array(self.allmatch_df['halpflux_sn'])>sncut))[0]
                self.filts[filt]['up_hamatch_filt'] = uphamatch
                self.filts[filt]['up_hamatch_df'] = self.allagn_df.iloc[uphamatch].copy()
                self.match_filts[filt]['up_hamatch_df'] = self.allmatch_df.iloc[uphamatch].copy()
                
                self.filts[filt]['down_hamatch_filt'] = downhamatch
                self.filts[filt]['down_hamatch_df'] = self.allagn_df.iloc[downhamatch].copy()
                self.match_filts[filt]['down_hamatch_df'] = self.allmatch_df.iloc[downhamatch].copy()
                for sub_filt in self.filts.keys():
                    if sub_filt == filt:
                        continue
                    if len(self.filts[sub_filt]['cut']) == 2:
                        
                        sub_mid_up = np.where((self.allagn_df[filt] > self.filts[filt]['cut'][0]) &
                                              (self.allagn_df[sub_filt] > self.filts[sub_filt]['cut'][0])&
                                              (self.allagn_df[sub_filt] <= self.filts[sub_filt]['cut'][1])
                                              )[0]
                        sub_mid_down = np.where((self.allagn_df[filt] <= self.filts[filt]['cut'][0])&
                                                (self.allagn_df[sub_filt] > self.filts[sub_filt]['cut'][0])&
                                                (self.allagn_df[sub_filt] <= self.filts[sub_filt]['cut'][1]))[0]

                        self.filts[filt]['up_mid_'+sub_filt+'_filt'] = sub_mid_up
                        self.filts[filt]['down_mid_'+sub_filt+'_filt'] = sub_mid_down
                        
                        self.filts[filt]['up_mid_'+sub_filt+'_df'] = self.allagn_df.iloc[sub_mid_up].copy()
                        self.filts[filt]['down_mid_'+sub_filt+'_df'] = self.allagn_df.iloc[sub_mid_down].copy()
                    ...
                    if sub_filt == 'sy2_liner_bool':
                        ...
                        sub_up_up = np.where((self.allagn_df[filt] > self.filts[filt]['cut'][0]) &
                                              (self.allagn_df[sub_filt] > self.filts[sub_filt]['cut'][0]))[0]
                        sub_up_down = np.where((self.allagn_df[filt] > self.filts[filt]['cut'][0])&
                                                (self.allagn_df[sub_filt] <= self.filts[sub_filt]['cut'][0]))[0]
                        
                        sub_down_up = np.where((self.allagn_df[filt] <= self.filts[filt]['cut'][0]) &
                                              (self.allagn_df[sub_filt] > self.filts[sub_filt]['cut'][0]))[0]
                        sub_down_down = np.where((self.allagn_df[filt] <= self.filts[filt]['cut'][0])&
                                                (self.allagn_df[sub_filt] <= self.filts[sub_filt]['cut'][0]))[0]
                        ...
                        self.filts[filt]['up_up_'+sub_filt+'_df'] = self.allagn_df.iloc[sub_up_up].copy()
                        self.filts[filt]['up_down_'+sub_filt+'_df'] = self.allagn_df.iloc[sub_up_down].copy()

                        self.filts[filt]['down_up_'+sub_filt+'_filt'] = sub_down_up
                        self.filts[filt]['down_down_'+sub_filt+'_filt'] = sub_down_down
                        
                        self.filts[filt]['down_up_'+sub_filt+'_df'] = self.allagn_df.iloc[sub_down_up].copy()
                        self.filts[filt]['down_down_'+sub_filt+'_df'] = self.allagn_df.iloc[sub_down_down].copy()
                ...
                mid = np.where((self.allagn_df[filt] > self.filts[filt]['cut'][0])&
                               (self.allagn_df[filt] <= self.filts[filt]['cut'][1]) )[0]                                    
                self.filts[filt]['mid'] = mid
                self.filts[filt]['mid_df'] = self.allagn_df.iloc[mid].copy()
                self.match_filts[filt]['mid_df'] = self.allmatch_df.iloc[mid].copy()
                for sub_filt in self.filts.keys():
                    if sub_filt == filt:
                        continue
                    if len(self.filts[sub_filt]['cut']) == 1:
                        
                        sub_up_mid = np.where((self.allagn_df[sub_filt] > self.filts[sub_filt]['cut'][0]) &
                                              (self.allagn_df[filt]>self.filts[filt]['cut'][0])&
                                              (self.allagn_df[filt]<=self.filts[filt]['cut'][1])
                                              )[0]
                        sub_down_mid = np.where((self.allagn_df[sub_filt] <= self.filts[sub_filt]['cut'][0])&
                                                (self.allagn_df[filt]>self.filts[filt]['cut'][0])&
                                                (self.allagn_df[filt]<=self.filts[filt]['cut'][1]))[0]
                        ...
                        self.filts[filt]['mid_up'+sub_filt+'_df'] = self.allagn_df.iloc[sub_up_mid].copy()
                        self.filts[filt]['mid_down'+sub_filt+'_df'] = self.allagn_df.iloc[sub_down_mid].copy()
        ...
        hamatch_filt = np.where(self.allmatch_df['halpflux_sn']>2)[0]
        self.match_filts['df_hamatch'] = self.allmatch_df.iloc[hamatch_filt].copy()            
        self.filts['df_hamatch'] = self.allagn_df.iloc[hamatch_filt].copy()            
    ...
    def bin_by_bpt(self, binsize=0.1):
        ...
        for i in range(len(xvals)-1):
            for j in range(len(yvals)-1):
                ...
                valid_sing = np.where((self.allagn_df.oiiihb >= yvals[j]) &
                                      (self.allagn_df.oiiihb < yvals[j+1]) &
                                      (self.allagn_df.niiha >= xvals[i]) &
                                      (self.allagn_df.niiha < xvals[i+1] )
                                      )[0]
                ...
                valid_clustering  = np.where((self.allagn_df.oiiihb.iloc[val1] >= yvals[j]) &
                                      (self.allagn_df.oiiihb.iloc[val1] < yvals[j+1]) &
                                      (self.allagn_df.niiha.iloc[val1] >= xvals[i]) &
                                      (self.allagn_df.niiha.iloc[val1] < xvals[i+1] )
                                      )[0]

                valid_clust1  = np.where((self.allagn_df.oiiihb.iloc[sy2_1] >= yvals[j]) &
                                      (self.allagn_df.oiiihb.iloc[sy2_1] < yvals[j+1]) &
                                      (self.allagn_df.niiha.iloc[sy2_1] >= xvals[i]) &
                                      (self.allagn_df.niiha.iloc[sy2_1] < xvals[i+1] )
                                      )[0]


                valid_clust2 = np.where((self.allagn_df.oiiihb.iloc[sf_1] >= yvals[j]) &
                                      (self.allagn_df.oiiihb.iloc[sf_1] < yvals[j+1]) &
                                      (self.allagn_df.niiha.iloc[sf_1] >= xvals[i]) &
                                      (self.allagn_df.niiha.iloc[sf_1] < xvals[i+1] )
                                      )[0]

                valid_clust3 = np.where((self.allagn_df.oiiihb.iloc[liner2_1] >= yvals[j]) &
                                      (self.allagn_df.oiiihb.iloc[liner2_1] < yvals[j+1]) &
                                      (self.allagn_df.niiha.iloc[liner2_1] >= xvals[i]) &
                                      (self.allagn_df.niiha.iloc[liner2_1] < xvals[i+1] )
                                      )[0]
                ...
                match_dist_clust1 = np.copy(self.mindists_best_sing_ord)[sy2_1]
                match_dist_clust2 = np.copy(self.mindists_best_sing_ord)[sf_1]
                match_dist_clust3 = np.copy(self.mindists_best_sing_ord)[liner2_1]
                ...
                if valid_clustering.size > 10:
                    distro_clustering_x = np.copy(self.allagn_df.offset_niiha.iloc[valid_clustering])
                    distro_clustering_y = np.copy(self.allagn_df.offset_oiiihb.iloc[valid_clustering])
                ...
                if valid_clust1.size > 10:
                    distro_clust1_x = np.copy(self.allagn_df.offset_niiha.iloc[valid_clust1])
                    distro_clust1_y = np.copy(self.allagn_df.offset_oiiihb.iloc[valid_clust1])
                ...
                if valid_clust2.size > 10:
                    distro_clust2_x = np.copy(self.allagn_df.offset_niiha.iloc[valid_clust2])
                    distro_clust2_y = np.copy(self.allagn_df.offset_oiiihb.iloc[valid_clust2])
                ...
                if valid_clust3.size > 10:
                    distro_clust3_x = np.copy(self.allagn_df.offset_niiha.iloc[valid_clust3])
                    distro_clust3_y = np.copy(self.allagn_df.offset_oiiihb.iloc[valid_clust3])
                ...
                if valid_sing.size > 10:
                    distro_full_x = np.copy(self.allagn_df.offset_niiha.iloc[valid_sing])
                    distro_full_y = np.copy(self.allagn_df.offset_oiiihb.iloc[valid_sing])
